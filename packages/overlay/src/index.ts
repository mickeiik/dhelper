...
class OverlayServiceImpl implements OverlayService {
  ...
  async createOverlay(options?: OverlayOptions): Promise<OverlayWindow> {
    ...
    const window = new BrowserWindow({
      ...
      width: bounds.width,
      height: bounds.height,
      transparent: validatedOptions.transparent ?? this.config.transparent,
      frame: false,
      alwaysOnTop: validatedOptions.alwaysOnTop ?? this.config.alwaysOnTop,
      skipTaskbar: true,
      resizable: false,
      movable: false,
      minimizable: false,
      maximizable: false,
      closable: true,
      focusable: !(validatedOptions.clickThrough ?? this.config.clickThrough),
    ...
    const clickThrough = validatedOptions.clickThrough ?? this.config.clickThrough;
    if (clickThrough) {
    ...
    const possiblePaths = this.config.htmlPaths;
    ...
    if (!overlayHtmlPath) {
      ...
      console.error(`[OverlayService] Critical error:`, error);
      throw error;
    }    
    ...
    const initializeOverlay = () => {
      ...
      window.webContents.send('overlay-init', {
        id: overlayId,
        options: {
          showInstructions: validatedOptions.showInstructions ?? this.config.ui.showInstructions,
          instructionText: validatedOptions.instructionText,
          timeout: validatedOptions.timeout ?? this.config.ui.overlayTimeout
        }
      });
    };
    ...
    setTimeout(() => {
      if (!initialized) {
        console.log(`[OverlayService] Force initializing overlay ${overlayId} after timeout`);
        safeInitialize();
      }
    }, 500);

    // Handle auto-close timeout
    const timeout = validatedOptions.timeout ?? this.config.ui.overlayTimeout;
...
export function createOverlayService(config: OverlayConfig): OverlayService {
  return new OverlayServiceImpl(config);
}
...
export type { Point, Rectangle, OverlayOptions, OverlayShape, OverlayText };